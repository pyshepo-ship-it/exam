...
const printElement = () => { };
import { gradeExam } from "./exam-grade.mjs";
/** إخفاء مفاتيح التصحيح عن واجهة الطالب — الاختبار الإلكتروني */
export function stripExamAnswers(exam) {
    return {
        ...exam,
...
function collectAnswerSpec(exam) {
    const spec = {};
...
function applyAnswerSpec(exam, spec) {
    return {
        ...exam,
        questions: (exam.questions || []).map(q => ({
            ...q,
...
function computeExamHash(examId, createdAt, specJson) {
...
export function sealExamForStudent(exam) {
 ...
 */
export function decodeSealForReview(token, examId) {
    try {
        const bin = atob(token.split("").reverse().join(""));
        const bytes = Uint8Array.from(bin, ch => ch.charCodeAt(0));
        const envelope = JSON.parse(new TextDecoder().decode(bytes));
        if (!envelope || envelope.eid !== examId || !envelope.sig)
            return {};
        const specJson = JSON.stringify(envelope.spec);
        if (computeExamHash(envelope.eid, envelope.ca, specJson) !== envelope.sig)
            return {};
...
export function gradeSealedExam(view, token, answers) {
    let spec = {};
    try {
...
export function toPublicExamCard(exam) {
    return {
        ...exam,
        questions: [],
    };
